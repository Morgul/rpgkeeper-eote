...
{
    //TODO: We only want to lower-case them in the event they're strings. If they are not, this will break.
    return function(modelVal){ return _.contains(modelVal.toLowerCase(), queryVal.toLowerCase()); };
} // end containsFilter

//----------------------------------------------------------------------------------------------------------------------

function search(model, queryObj)
{
    var query = {};
    var filters = {};
    _.forIn(queryObj, function(value, key)
    {
        ...
        {
            value = value.substr(2);
            filters[key] = containsFilter(castParam(type, value));
        }
